...
// HasLastSeenAt applies the HasEdge predicate on the "last_seen_at" edge.
func HasLastSeenAt() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasLastSeenAtWith applies the HasEdge predicate on the "last_seen_at" edge with a given conditions (other predicates).
func HasLastSeenAtWith(preds ...predicate.LastSeenOnline) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newLastSeenAtStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
