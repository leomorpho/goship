...
func (iq *ImageQuery) QuerySizes() *ImageSizeQuery {
	query := (&ImageSizeClient{config: iq.config}).Query()
...
func (iq *ImageQuery) WithSizes(opts ...func(*ImageSizeQuery)) *ImageQuery {
	query := (&ImageSizeClient{config: iq.config}).Query()
...
func (iq *ImageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Image, error) {
	...
	if query := iq.withSizes; query != nil {
		if err := iq.loadSizes(ctx, query, nodes,
			func(n *Image) { n.Edges.Sizes = []*ImageSize{} },
			func(n *Image, e *ImageSize) { n.Edges.Sizes = append(n.Edges.Sizes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *ImageQuery) loadSizes(ctx context.Context, query *ImageSizeQuery, nodes []*Image, init func(*Image), assign func(*Image, *ImageSize)) error {
	...
	query.withFKs = true
	query.Where(predicate.ImageSize(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(image.SizesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.image_sizes
		if fk == nil {
			return fmt.Errorf(`foreign-key "image_sizes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "image_sizes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
