...
// QueryLastSeenAt chains the current query on the "last_seen_at" edge.
func (uq *UserQuery) QueryLastSeenAt() *LastSeenOnlineQuery {
	query := (&LastSeenOnlineClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithLastSeenAt tells the query-builder to eager-load the nodes that are connected to
// the "last_seen_at" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithLastSeenAt(opts ...func(*LastSeenOnlineQuery)) *UserQuery {
	query := (&LastSeenOnlineClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withLastSeenAt = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withLastSeenAt; query != nil {
		if err := uq.loadLastSeenAt(ctx, query, nodes,
			func(n *User) { n.Edges.LastSeenAt = []*LastSeenOnline{} },
			func(n *User, e *LastSeenOnline) { n.Edges.LastSeenAt = append(n.Edges.LastSeenAt, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadLastSeenAt(ctx context.Context, query *LastSeenOnlineQuery, nodes []*User, init func(*User), assign func(*User, *LastSeenOnline)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.LastSeenOnline(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.LastSeenAtColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_last_seen_at
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_last_seen_at" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_last_seen_at" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
