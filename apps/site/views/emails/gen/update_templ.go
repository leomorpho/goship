...
var emailUpdateGoTemplate = template.Must(template.New("content").Parse(`
...
<html xmlns="http://www.w3.org/1999/xhtml" xmlns="http://www.w3.org/1999/xhtml" style="color-scheme: light dark; supported-color-schemes: light dark;">
  ...
  <body style="width: 100% !important; height: 100%; -webkit-text-size-adjust: none; font-family: 'Nunito Sans', Helvetica, Arial, sans-serif; background-color: #F2F4F6; color: #51545E; margin: 0;" bgcolor="#F2F4F6">
    <span class="preheader" style="display: none !important; visibility: hidden; mso-hide: all; font-size: 1px; line-height: 1px; max-height: 0; max-width: 0; opacity: 0; overflow: hidden;">Here are the latest updates for you.</span>
          ...
          <table class="email-content" width="100%" cellpadding="0" cellspacing="0" role="presentation" style="width: 100%; -premailer-width: 100%; -premailer-cellpadding: 0; -premailer-cellspacing: 0; margin: 0; padding: 0;">
            <tr>
              <td class="email-masthead" style="word-break: break-word; font-family: 'Nunito Sans', Helvetica, Arial, sans-serif; font-size: 16px; text-align: center; padding: 25px 0;" align="center">
                <a href="{{.Domain}}" class="f-fallback email-masthead_name" style="color: #A8AAAF; font-size: 16px; font-weight: bold; text-decoration: none; text-shadow: 0 1px 0 white;">
                  {{.AppName}}
                </a>
              </td>
            </tr>
                  ...
                  <tr>
                    <td class="content-cell" style="word-break: break-word; font-family: 'Nunito Sans', Helvetica, Arial, sans-serif; font-size: 16px; padding: 45px;">
                      <div class="f-fallback">
                        <h1 style="margin-top: 0; color: #333333; font-size: 22px; font-weight: bold; text-align: left;" align="left">Hello from {{.AppName}}, {{.SelfName}}!</h1>
                        <p style="font-size: 16px; line-height: 1.625; color: #51545E; margin: .4em 0 1.1875em;">Here are your latest updates:</p>

                        {{if .NumNewNotifications}}
                        <h2 style="margin-top: 0; color: #333333; font-size: 18px; font-weight: bold; text-align: left;" align="left">
                          {{if eq .PartnerName ""}}
                            You have {{.NumNewNotifications}} notifications.
                          {{else}}
                            You have {{.NumNewNotifications}} notifications from {{.PartnerName}}.
                          {{end}}
                        </h2>
                        {{end}}

                        {{if .QuestionsAnsweredByFriendButNotSelf}}
                        <h2 style="margin-top: 0; color: #333333; font-size: 18px; font-weight: bold; text-align: left;" align="left">{{.QuestionsAnsweredByFriendButNotSelfTitle}}</h2>
                        <ul style="font-size: 16px; line-height: 1.625; color: #51545E; margin: .4em 0 1.1875em;">
                          {{range .QuestionsAnsweredByFriendButNotSelf}}
                          <li><a href="{{.WriteAnswerURL}}" style="color: #3869D4; text-decoration: none;">{{.Question}}</a></li>
                          {{end}}
                        </ul>
                        {{end}}

                        {{if .QuestionsNotAnsweredInSocialCircle}}
                        <h2 style="margin-top: 0; color: #333333; font-size: 18px; font-weight: bold; text-align: left;" align="left">New Questions for You:</h2>
                        <ul style="font-size: 16px; line-height: 1.625; color: #51545E; margin: .4em 0 1.1875em;">
                          {{range .QuestionsNotAnsweredInSocialCircle}}
                          <li><a href="{{.WriteAnswerURL}}" style="color: #3869D4; text-decoration: none;">{{.Question}}</a></li>
                          {{end}}
                        </ul>
                        {{end}}

                        <p style="font-size: 16px; line-height: 1.625; color: #51545E; margin: .4em 0 1.1875em;">If you have any questions, feel free to <a href="mailto:{{.SupportEmail}}" style="color: #3869D4;">email our customer success team</a>.</p>
                        ...
                        <table class="body-sub" role="presentation" style="margin-top: 25px; padding-top: 25px; border-top-width: 1px; border-top-color: #EAEAEC; border-top-style: solid;">
                          <tr>
                            <td style="word-break: break-word; font-family: 'Nunito Sans', Helvetica, Arial, sans-serif; font-size: 16px;">
                              {{if .UnsubscribeDailyUpdatesLink}}
                              <p class="f-fallback sub" style="font-size: 13px; line-height: 1.625; color: #51545E; margin: .4em 0 1.1875em;">
                                <a href="{{.UnsubscribeDailyUpdatesLink}}" style="color: #3869D4;">Unsubscribe from daily updates</a>
                              </p>
                              {{end}}
                              {{if .UnsubscribePartnerActivityLink}}
                              <p class="f-fallback sub" style="font-size: 13px; line-height: 1.625; color: #51545E; margin: .4em 0 1.1875em;">
                                <a href="{{.UnsubscribePartnerActivityLink}}" style="color: #3869D4;">Unsubscribe from partner updates</a>
                              </p>
                              {{end}}
                            </td>
                          </tr>
...
func EmailUpdate(page *ui.Page) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		if data, ok := page.Data.(viewmodels.EmailUpdate); ok {
