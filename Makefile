...
.PHONY: dev
dev: ## Compatibility wrapper for ship dev (kept for muscle memory and editor tasks)
...
.PHONY: test-doc-drift
test-doc-drift: ## Verify README/LLM/MCP/canonical architecture entry docs stay aligned
	bash tools/scripts/check-doc-drift.sh
