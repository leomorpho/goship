...
func TestGenerateModelIntegration_WithFieldsWritesSchemaAndNextSteps(t *testing.T) {
	...
	code := cli.Run([]string{"make:model", "BlogPost", "title:string", "published_at:time", "is_live:bool"})
	if code != 0 {
		t.Fatalf("exit code = %d, stderr=%s", code, errOut.String())
	}

	schemaPath := filepath.Join(root, "apps", "goship", "db", "schema", "blog_post.go")
...
func TestGenerateModelIntegration_ForceOverwritesSchema(t *testing.T) {
	...
	schemaPath := filepath.Join(root, "apps", "goship", "db", "schema", "post.go")
	...
	code := cli.Run([]string{"make:model", "Post", "title:string", "--force"})
	if code != 0 {
		t.Fatalf("exit code = %d, stderr=%s", code, errOut.String())
	}
