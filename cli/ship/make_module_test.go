...
func TestParseMakeModuleArgs(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			check: func(t *testing.T, opts moduleMakeOptions) {
				if opts.Path != filepath.Join("pkg", "modules") {
...
func TestRunMakeModule_DryRun(t *testing.T) {
	...
	code := cli.Run([]string{"make:module", "EmailSubscriptions", "--dry-run"})
	if code != 0 {
	...
	if !strings.Contains(out.String(), "pkg/modules/emailsubscriptions/go.mod") {
...
func TestRunMakeModule_Integration(t *testing.T) {
	...
	code := cli.Run([]string{"make:module", "EmailSubscriptions"})
	if code != 0 {
		t.Fatalf("exit=%d stderr=%s", code, errOut.String())
	}

	moduleDir := filepath.Join(root, "pkg", "modules", "emailsubscriptions")
	required := []string{
