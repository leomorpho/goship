...
func (c CLI) runMakeModule(args []string) int {
	...
	if err != nil {
		fmt.Fprintf(c.Err, "invalid make:module arguments: %v\n", err)
		return 1
	}

	tokens := splitWords(opts.Name)
	if len(tokens) == 0 {
		fmt.Fprintln(c.Err, "usage: ship make:module <Name> [--path pkg/modules] [--module-base github.com/leomorpho/goship-modules] [--dry-run] [--force]")
		return 1
	}
	...
	if pathExists(moduleDir) && !opts.Force {
		fmt.Fprintf(c.Err, "refusing to overwrite existing module directory: %s (use --force)\n", moduleDir)
		return 1
	}
	...
	if opts.DryRun {
		fmt.Fprintf(c.Out, "Module scaffold plan (dry-run):\n- module: %s\n- dir: %s\n", modulePath, moduleDir)
		for p := range files {
	...
	for p, content := range files {
		if err := os.MkdirAll(filepath.Dir(p), 0o755); err != nil {
			fmt.Fprintf(c.Err, "failed to create directory for %s: %v\n", p, err)
			return 1
		}
	...
	fmt.Fprintf(c.Out, "Generated module scaffold at %s (%s)\n", moduleDir, modulePath)
	return 0
}

func parseMakeModuleArgs(args []string) (moduleMakeOptions, error) {
	opts := moduleMakeOptions{
		Path:       filepath.Join("pkg", "modules"),
		ModuleBase: "github.com/leomorpho/goship-modules",
	}
	if len(args) == 0 {
		return opts, errors.New("usage: ship make:module <Name> [--path pkg/modules] [--module-base github.com/leomorpho/goship-modules] [--dry-run] [--force]")
	}
