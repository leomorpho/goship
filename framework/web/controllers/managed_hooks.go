...
func NewManagedHooksRoute(ctr ui.Controller, deps ManagedHooksDeps) ManagedHooksRoute {
	nowFn := deps.Now
	if nowFn == nil {
		nowFn = time.Now
	}
	return ManagedHooksRoute{
		Controller:    ctr,
...
func (m ManagedHooksRoute) GetRuntimeStatus(ctx echo.Context) error {
	if err := m.requireManagedMode(); err != nil {
		return err
	}

	cfg := m.Controller.Container.Config
...
func (m ManagedHooksRoute) StartBackup(ctx echo.Context) error {
	if err := m.requireManagedMode(); err != nil {
		return err
	}
...
func (m ManagedHooksRoute) StartRestore(ctx echo.Context) error {
	if err := m.requireManagedMode(); err != nil {
		return err
	}
...
func (m ManagedHooksRoute) requireManagedMode() error {
	if m.Controller.Container == nil || m.Controller.Container.Config == nil || !m.Controller.Container.Config.Managed.Enabled {
