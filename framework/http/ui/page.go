...
import (
	...
	frameworkpage "github.com/leomorpho/goship/v2/framework/http/page"
	i18nmodule "github.com/leomorpho/goship/v2/modules/i18n"
)
...
func NewPage(ctx echo.Context) Page {
	...
	if u := ctx.Get(authcontext.AuthenticatedUserIDKey); u != nil {
		...
		if ok && userID > 0 {
			...
			if userName, ok := ctx.Get(authcontext.AuthenticatedUserNameKey).(string); ok {
				p.AuthUser.Name = userName
			}
			if userEmail, ok := ctx.Get(authcontext.AuthenticatedUserEmailKey).(string); ok {
				p.AuthUser.Email = userEmail
			}
		}
		if fullyOnboarded := ctx.Get(authcontext.ProfileFullyOnboarded); fullyOnboarded != nil {
