...
func PrintRootHelp(w io.Writer) {
	...
	fmt.Fprintln(w, "  ship destroy resource:<name>             Remove generator-managed resource scaffold")
	...
	fmt.Fprintln(w, "  ship upgrade --to <version> [--dry-run]          Show upgrade readiness and planned rewrites")
	fmt.Fprintln(w, "  ship upgrade apply --to <version> [--dry-run]    Apply deterministic upgrade rewrites")
	fmt.Fprintln(w)
...
func PrintMakeHelp(w io.Writer) {
	...
	fmt.Fprintln(w, "  ship make:controller <Name|NameController> [--actions index,show,create,update,destroy] [--auth public|auth] [--wire]                    Generate a controller with optional route wiring (starter-safe with generated CRUD semantics)")
	...
	fmt.Fprintln(w, "  ship make:locale <code>                                                                                                                      Generate locale file from baseline keys (requires existing locale baseline)")
	fmt.Fprintln(w, "  ship make:event <TypeName> [--force]                                                                                                        Generate a domain event type")
	fmt.Fprintln(w, "  ship make:island <Name>                                                                                                                      Generate a frontend island scaffold (starter-safe)")
	fmt.Fprintln(w, "  ship make:job <Name>                                                                                                                         Generate a background job scaffold (framework workspace only for now)")
	fmt.Fprintln(w, "  ship make:mailer <Name>                                                                                                                      Generate a mailer scaffold (framework workspace only for now)")
	fmt.Fprintln(w, "  ship make:schedule <Name> --cron \"<expr>\"                                                                                                   Insert a scheduled job entry (framework workspace only for now)")
	fmt.Fprintln(w, "  ship make:command <Name>                                                                                                                     Generate an app CLI command (framework workspace only for now)")
	fmt.Fprintln(w, "  ship make:module <Name> [--path modules] [--module-base github.com/leomorpho/goship-modules] [--dry-run] [--force]                     Generate a standalone module scaffold (framework authoring only)")
}
