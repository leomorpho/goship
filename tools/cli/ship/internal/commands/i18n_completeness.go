...
var (
	i18nCountCallPattern  = regexp.MustCompile(`(?:I18n\.TC|i18n\.TC)\s*\(\s*(?:[^,]+,\s*)?"([a-zA-Z0-9._-]+)"`)
	i18nSelectCallPattern = regexp.MustCompile(`(?:I18n\.TS|i18n\.TS)\s*\(\s*(?:[^,]+,\s*)?"([a-zA-Z0-9._-]+)"`)
)
...
func CollectI18nCompletenessIssues(root string) ([]I18nCompletenessIssue, error) {
	...
	countBases, err := collectI18nCallBases(root, i18nCountCallPattern)
	if err != nil {
		return nil, err
	}
	selectBases, err := collectI18nCallBases(root, i18nSelectCallPattern)
	if err != nil {
		return nil, err
	}
	...
	for _, locale := range locales {
		...
		for _, base := range countBaseList {
			...
			if strings.TrimSpace(flat[otherKey]) == "" {
				issues = append(issues, newI18nCompletenessIssue(locale, rel, "plural_missing_other", base, fmt.Sprintf("missing required plural fallback key %s for TC usage", otherKey)))
			}
			...
			if !hasCountVariant {
				issues = append(issues, newI18nCompletenessIssue(locale, rel, "plural_missing_variant", base, fmt.Sprintf("missing non-other plural variant for TC usage of %s", base)))
			}
		}
		...
		for _, base := range selectBaseList {
			otherKey := base + ".other"
			if strings.TrimSpace(flat[otherKey]) == "" {
				issues = append(issues, newI18nCompletenessIssue(locale, rel, "select_missing_other", base, fmt.Sprintf("missing required select fallback key %s for TS usage", otherKey)))
			}
			prefix := base + "."
			hasVariant := false
			...
			if !hasVariant {
				issues = append(issues, newI18nCompletenessIssue(locale, rel, "select_missing_variant", base, fmt.Sprintf("missing non-other select variant for TS usage of %s", base)))
			}
		}
	}
...
func newI18nCompletenessIssue(locale, file, kind, base, message string) I18nCompletenessIssue {
