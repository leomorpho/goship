...
func TestDocs_DBExportAndRuntimeReportContractsStayInSync_RedSpec(t *testing.T) {
	...
	for _, token := range []string{
		...
		"`ship runtime:report --json` -> machine-readable runtime capability report covering active profile, adapters, process plan, web features, DB runtime metadata, managed-key sources, per-module adoption metadata, and a versioned handshake envelope",
	} {
	...
	for _, token := range []string{
		...
		"`ship runtime:report --json` now exposes the effective profile, adapters, process plan, web features, DB runtime metadata, managed-key sources, per-module adoption metadata, and a versioned handshake envelope for orchestration preflight.",
	} {
