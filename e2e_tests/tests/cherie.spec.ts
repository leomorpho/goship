...
/*
NOTE that before running these tests, you need to have Chérie running.
...
test.describe("Committed relationship workflows", () => {
  ...
  test("Self convo view", async ({ browser }) => {
    ...
    await pageUser1.getByRole("link", { name: "Logo Chérie" }).click();
  ...
  test("Waiting on you view", async ({ browser }) => {
    ...
    await pageUser2.getByRole("link", { name: "Logo Chérie" }).click();
  ...
  test("Registration email", async ({ page }) => {
    ...
    await expect(
      page
        ...
        .getByRole("heading", { name: "Welcome to Chérie!" })
    ).toBeVisible();
    // await expect(
