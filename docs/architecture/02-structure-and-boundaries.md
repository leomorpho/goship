...
<!-- FRONTEND_SYNC: Landing capability explorer in apps/site/views/web/pages/landing_page.templ links here for Views and Server UI. Keep both landing copy and this doc aligned. -->
...
- `apps/site/web/controllers`: handlers
- `apps/site/web/wiring.go`: HTTP wiring helpers (middleware/static/dependencies)
- `apps/site/foundation`: app composition container and app-specific runtime adapters
- `apps/site/views`: templ components/layouts/pages/emails
- `apps/site/views/**/gen`: generated templ Go files (same package names as source dirs)
...
- Route handlers in `apps/site/web/controllers` are the controller layer (Rails/Laravel equivalent at the HTTP boundary).
...
- App business logic should live in app-scoped domain/service packages under `apps/site/...`.
...
- Moved routes from `pkg/routing/routes` to `apps/site/web/controllers`.
- Moved templ views from `templates` to `apps/site/views`.
...
1. It has no hard dependency on `apps/site/web/controllers` or app views/templates.
