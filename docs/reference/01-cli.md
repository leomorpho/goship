...
- `ship runtime:report --json` -> machine-readable runtime capability report covering active profile, adapters, process plan, web features, DB runtime metadata, managed-key sources, and a versioned handshake envelope; downstream staged rollout and canary orchestration must compose those runtime facts into `staged-rollout-decision-v1` instead of inventing a second runtime-specific decision payload; `decision=canary` requires a populated `canary` object, while `verification` evidence (`checks`, `evidence_refs`, `verified_by`) must remain explicit in the decision artifact
