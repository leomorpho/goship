...
- `ship module:add <name>` now prints an explicit install contract summary grouped by `routes`, `config`, `assets`, `jobs`, `templates`, and `migrations` so install/remove impact is visible before review
