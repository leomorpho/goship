...
- `ship runtime:report --json` also emits backup/restore contract metadata (`backup.manifest_version`, `backup.restore_evidence.*`, and canonical `record_links` field names) so managed consumers can parse backup evidence contracts without duplicating runtime constants.
- `ship runtime:report --json` also emits a rollout/promotion decision-input contract (`decision_input`) with stable schema/version fields (`staged-rollout-decision-v1`, `promotion-state-machine-v1`, `policy_input_version`) and explicitly marks orchestration as external (`orchestration_embedded=false`).
  - module adoption now includes a stable first-party baseline with `installed=false` when those modules are not enabled, so control-plane tooling can compare adoption consistently across runtimes
...
- canonical first-class installable batteries on the current generated-app v1 path are `jobs`, `storage`, and `emailsubscriptions`; this baseline is authored in `config/module-surface.yaml` and verified against `docs/architecture/11-module-surface-reset.md`
- broader reference-set batteries for module-surface reset work remain `jobs`, `storage`, `emailsubscriptions`, and `paidsubscriptions`, but `notifications` and `paidsubscriptions` are not yet installable on the canonical generated-app path
...
- upgrade output now includes `execution_steps` classification (`automatic`, `manual`, `blocked`) so command/report consumers can distinguish rewrite automation, operator-required actions, and hard blockers consistently
