...
- current first-class installable batteries include `notifications`, `paidsubscriptions`, `emailsubscriptions`, `realtime`, `jobs`, and `storage`; `realtime` captures the canonical runtime startup and route contract seams, and `storage` exposes the canonical `core.BlobStorage` seam as a standalone battery package under `modules/storage`
