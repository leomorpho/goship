...
Scope note:
- this workflow currently targets the **framework workspace**.
- the minimal starter scaffold generated by default `ship new` rejects `make:job` and `make:schedule` with a clear error for now.
