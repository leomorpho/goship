...
- the current supported first-party battery set for the v1 generated-app path is: `jobs`, `storage`, `emailsubscriptions`, and `paidsubscriptions`.
- `notifications` remains part of the broader module-surface reset roadmap, but it is not yet installable on the canonical generated-app path.
