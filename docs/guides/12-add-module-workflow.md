...
Scope note:
- this workflow is for the framework workspace / module-authoring path.
- the minimal starter scaffold generated by default `ship new` still rejects `ship module:add` and does not support `make:module`.
- the current supported first-party battery set for the v1 generated-app path is: `jobs`, `storage`, `emailsubscriptions`, `paidsubscriptions`, and `notifications`.
- `notifications` remains the temporary bridge battery while the split-battery plan evolves, but it is now installable on the canonical generated-app path.
