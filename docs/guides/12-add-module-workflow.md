...
- the current supported first-party battery set for the v1 generated-app path is: `jobs`.
- `notifications`, `paidsubscriptions`, `emailsubscriptions`, and `storage` remain part of the broader module-surface reset roadmap, but they are not yet installable on the canonical generated-app path.
