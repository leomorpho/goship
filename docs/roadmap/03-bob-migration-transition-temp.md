...
- [x] Keep Ent only as temporary compatibility during migration.
- [x] Remove Ent entirely when parity is complete.

## Architecture Target (Definition of Done)

- [x] Core app owns only core DB artifacts.
- [x] Each installable module owns its DB artifacts end-to-end.
- [x] `ship db:*` commands run Goose + Bob in a deterministic way.
- [x] No module DB package imports from app core DB package.
- [x] No app feature logic depends directly on Ent.
...
## Current Baseline Snapshot (As of 2026-03-06)

- [x] Migration runtime is Goose + Bob (Ent removed from runtime/toolchain paths).
...
- [x] `db:generate` exists for core + installed modules.
- [ ] `make:migration` alias does not exist; `db:make` is current canonical command.
- [x] Goose runtime package/invocation path exists.
...
- [x] Confirm no runtime code paths require Ent.
- [x] Confirm migrations and generators run fully via Goose + Bob.

### 5.1a Runtime Ent cutover backlog (execution order)
- [x] `app/profile`: extract SQL/Bob stores for profile reads/writes/media and remove direct Ent runtime dependency.
...
- [x] Remove Ent dependencies from `go.mod`/`go.work`.

Acceptance criteria:
- [x] `ship doctor` has no Ent-presence expectations.
- [ ] Full pre-commit and integration suite green without Ent.
- [x] Clean-room build/test verifies no hidden Ent dependency remains.
...
- 2026-03-06: added CI clean-room verification job (`cleanroom_bob_verification`) backed by `tools/scripts/test-cleanroom.sh`; it scaffolds a fresh app, runs Goose migrations on SQLite, runs Bob generation, and executes `go test ./...` to catch hidden cross-repo/toolchain coupling.
