...
- [ ] `app/jobs`: move notifications/mail/email-update DB access behind SQL/Bob stores.
- [ ] `framework/repos/storage`: remove Ent-typed boundaries (`*ent.Image`) from runtime interfaces.
- [ ] `app/foundation`: remove Ent-default auth/container runtime paths (keep temporary test-only seams only as needed).
- [ ] `cmd/*` + app wiring: validate all module runtime deps run DB-first without Ent at composition root.
