...
1. It has no hard dependency on app route/view packages (`apps/site/web/controllers`, templ views).
...
Test evidence:
- `go test ./apps/site/foundation -run 'Test(NewContainer|ContainerShutdownNilSafe)$'`
...
Status: `completed`
Done when:
- canonical router entrypoint remains `apps/site/router.go`;
- route declarations are centralized in `apps/site/router.go`;
...
Status: `completed`
Done when:
- `ship make:resource ... --wire` inserts generated snippet into `apps/site/router.go`;
...
Status: `completed`
Done when:
- generator ensures `RouteName<Resource>` constant is present in `apps/site/web/routenames/routenames.go`;
...
- notifier publish/subscribe path in `apps/site/app/notifications/notifier.go` now uses `core.PubSub`.
