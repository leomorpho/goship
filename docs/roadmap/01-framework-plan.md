...
- executable normalization specs now pin both the `memory`/`otter` adapter alias contract and the positive-TTL round-up rule so adapter choice does not change expiry semantics.
