...
## Acceptance Criteria
- [ ] `factories.User.Create(t, db)` inserts a user and returns it with its DB ID set. Overrides work: `factories.User.Create(t, db, factories.WithAdminRole)` creates an admin. Sequence ensures unique emails across test runs. `go test ./...` passes.
