...
## Acceptance Criteria
- [ ] `ship verify` runs the full pipeline. Fails fast on first error with clear output. `ship verify --json` outputs structured result. `ship verify --skip-tests` skips the test step.
