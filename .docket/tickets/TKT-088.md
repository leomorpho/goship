...
## Acceptance Criteria
- [ ] User can enable 2FA from profile. QR code works with Google Authenticator. Login with 2FA enabled requires the 6-digit code. Backup codes work. Invalid codes are rejected. `ship module:add 2fa` installs the module. `ship verify` passes.
