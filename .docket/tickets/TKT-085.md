...
## Acceptance Criteria
- [ ] Worker process starts the cron scheduler. A schedule registered in `schedules.go` fires at the correct time and enqueues a job. `ship make:schedule DailyReport --cron "0 9 * * *"` inserts a new entry at the marker. `go build ./...` passes.
