...
updated_at: "2026-03-11T07:11:00Z"
...
- Replaced the Viper/YAML loader in `config/config.go` with a `cleanenv`-based env loader.
- Preserved existing `PAGODA_*` environment variable names used in deploy manifests and added a compatibility alias for `PAGODA_DATABASE_DATABASE`.
- Moved runtime/profile defaults and process-profile resolution into code so config no longer depends on Viper merges.
- Removed Viper from `go.mod` and added `github.com/ilyakaznacheev/cleanenv`.

## Verification
- `go test ./config ./app/foundation ./framework/core/adapters ./tools/cli/ship/internal/runtime ./tools/cli/ship/internal/policies ./cmd/web ./cmd/worker`
- `ship doctor`
