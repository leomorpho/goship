...
## Acceptance Criteria
- [ ] Setting `AI_DRIVER=openrouter` + `OPENROUTER_API_KEY=...` routes all AI calls through OpenRouter. Model constants work as model identifiers. OpenAI driver works with OpenAI directly. All three drivers implement `ai.Provider` and pass the same interface tests.
