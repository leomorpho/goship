...
Note: `ship verify --skip-tests` is currently blocked by pre-existing repository-level doctor errors (`DX021` in `modules/admin/routes.go` and existing `DX023` templ comment coverage), not by this ticket's i18n changes.

## Follow-up Program (v2)
This ticket established i18n baseline only. Follow-up work is tracked in umbrella `TKT-126`:
1. Migration and LLM automation: `TKT-116` (+ leaf tickets `TKT-127`, `TKT-128`, `TKT-129`)
2. Enforcement and strict mode: `TKT-117` (+ `TKT-130`, `TKT-131`, `TKT-132`)
3. Pluggable contract and disabled mode: `TKT-118` (+ `TKT-133`, `TKT-134`, `TKT-135`)
4. Canonical locale format: `TKT-119` (+ `TKT-136`)
5. Plurals and edge cases: `TKT-120` (+ `TKT-137`, `TKT-138`)
6. Islands/API parity: `TKT-121` (+ `TKT-139`, `TKT-140`)
7. Starter onboarding: `TKT-122`
8. Language switcher persistence wiring: `TKT-125` (+ `TKT-141`, `TKT-142`)
9. Program docs and long-term packs: `TKT-123`, `TKT-124`
