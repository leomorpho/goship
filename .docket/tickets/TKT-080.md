...
## Acceptance Criteria
- [ ] All responses include the security headers. CSP nonce is set per-request. Dev config allows Vite HMR without CSP violations. `curl -I http://localhost:8080/` shows all headers. securityheaders.com scan on staging returns A grade.
