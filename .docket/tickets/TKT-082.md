...
## Acceptance Criteria
- [ ] `container.Mailer.Send(ctx, email)` sends an email via the configured driver. Log driver outputs to stdout in test/dev. SMTP and Resend drivers compile and have unit tests (mock the network call). `ship verify` passes.
