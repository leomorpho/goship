...
## Acceptance Criteria
- [ ] Auth module publishes events. A subscriber registered in `app/foundation/container.go` receives them. `Subscribe[events.UserLoggedIn]` works with type inference. Async variant enqueues correctly. Tests verify publish → handler called.
