...
```bash
<agent-cli> mcp add --scope user github -- npx -y @modelcontextprotocol/server-github
<agent-cli> mcp list
```
...
```bash
<agent-cli> mcp add --scope user ship -- ~/.local/bin/ship-mcp
```
...
```bash
<agent-cli> mcp add --scope user playwright -- npx -y @playwright/mcp
```
...
- If the route is genuinely unknown: use the GoShip MCP `ship_routes` tool to list all registered routes, then navigate candidate routes and check via `browser_snapshot` which `data-component` values appear on the page.
