...
func TestPwaPushService_SQLStore_Lifecycle(t *testing.T) {
	...
	svc := NewSQLPwaPushService(db, "sqlite3", "pub", "priv", "test@example.com")
	...
	ok, err := svc.HasEndpointRegistered(ctx, 55, "https://example.invalid/sub-1")
	require.NoError(t, err)
	require.True(t, ok)
...
func TestFcmPushService_SQLStore_Lifecycle(t *testing.T) {
	db := openPushStoresTestDB(t)
	svc, err := NewSQLFcmPushService(db, "sqlite3", nil)
	require.NoError(t, err)
	...
	ok, err := svc.HasTokenRegistered(ctx, 77, "token-1")
	require.NoError(t, err)
	require.True(t, ok)
